...
void playersBet(int num, int valid[], int money[], int* bet, char names[][100], int jj) {
    ...
    int condi = 0, condi1, mu = num, yu = 0, maximuimBet = 10000;
    for (int h = 0; h < num; h++) {
        ...
        for (int g = 0; g < num; g++) {
            if (jj == 1 && valid[g] != 1) {
                ...
                if (bett == 'f') {
                    ...
                    scanf("%d", &temp1);
                    while (temp1 > money[g] || temp1 <= yu) {
                        if (temp1 > money[g]) {
                            printf("Your re-raise is more than your available money.\n");
                            printf("How much you want to re-raise? ");
                            scanf("%d", &temp1);
                        }
                        else if (temp1 <= yu) {
                            printf("Your re-raise is must be higher.\n");
                            printf("How much you want to re-raise? ");
                            scanf("%d", &temp1);
                        }
                    }
...
void gradestatics(int game[][2], int points[10], int num) {
    ...
    game[sa][1]++;
    for (int i = 0; i < num; i++) { //if there is another player with same points
        if (i != sa) {
            if (max == points[i]) {
                game[i][1]++;
            }
        ...
        for (int i = 0; i < x; i++) {
            ...
            if (valid[i] != 1) {
                ...
                if (temp == 'k' || me == imp - 1 || go) { //i tried to put this in a function but it needed 12 variables to be passed and this would just make the function look wierd so i keep it here
                    ...
                    else { //distribute the money for other players and checks the winners and give the bet to the winner either if its just one winner or a tie
                        int pointss[10];
                        for (int j = 0; j < x; j++) {
                            if (valid[j] != 1) {
